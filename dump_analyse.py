...
#         sizes = sk.find_all('label', class_='OutOfStock') + \
#             sk.find_all('label', class_='InStock')
#         img = sk.find(
#             'div', class_='product__media media media--transparent gradient global-media-settings').find('img').get('src')
#         size = [x.text.strip() for x in sizes]
#         new_snkr = Sneaker(name=sk.find('title').text.strip().replace('\n', ''), url=i, img=img, price=sk.find(
#             'span', class_='price-item price-item--regular').text.strip().replace('\n', ''))
#         new_size = ''
#         for k in sizes:
...
#         sk = bs(req.content, 'lxml')
#         img = sk.find(
#             'div', class_='product__media media media--transparent media__height').find('img').get('src')
#         new_snkr = Sneaker(name=sk.find('title').text.strip().replace('\n', ''), url=i, price=sk.find(
#             'span', class_='price-item price-item--regular').text.strip().replace('\n', ''), img=img)
#         new_size = ''
...
#         sk = bs(req.content, 'lxml')
#         img = sk.find(
#             'img', class_='Image--fadeIn lazyautosizes Image--lazyLoaded')
...
#         snkr = Sneaker.query.filter_by(name=cd.find(
#             'h1', class_='ProductMeta__Title Heading u-h2').text.strip().replace('\n', '')).first()
#         img = cd.find('div', class_='Product__SlideItem Product__SlideItem--image Carousel__Cell is-selected').find(
#             'img').get('data-src').replace('{width}', '1400')
#         snkr.img = img
#         db.session.add(snkr)
#         db.session.commit()
#     except Exception as e:
